...
------------------------------------------
-- Define a global variable to store the custom message
_G.custom_status_message = ""

-- Function to update the custom message based on mode and file path
local function update_status_message()
  local file_path = vim.fn.expand('%:p')
  local current_mode = vim.fn.mode()

  -- Only set the message if in Normal mode and file path matches
  if current_mode == 'n' and file_path:match(vim.fn.expand('$HOME') .. '/Documents/Obsidian_Notes') then
    _G.custom_status_message = "PUSH NOTES!"
  end
end

-- Function to clear the custom message
local function clear_status_message()
  _G.custom_status_message = ''
end

-- Call these functions on BufWritePost and ModeChanged events
